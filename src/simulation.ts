...
export const VEHICLES: VehicleDefinition[] = [
  { id: 'bike', name: 'ホバーバイク', tag: 'KAZE / LAND', description: '路地をすり抜ける、一人乗りの浮遊バイク。', color: '#66ffe1', maxSpeed: 16, acceleration: 9, turnRate: 1.8, radius: .7, home: [2, 24] },
  { id: 'truck', name: '電動軽トラック', tag: 'MINATO / CARGO', description: '水産市場の働き者。ゆっくり街を巡る。', color: '#ffc07a', maxSpeed: 10, acceleration: 5, turnRate: 1.2, radius: 1.25, home: [35, 23] },
  { id: 'boat', name: '水上タクシー', tag: 'NAMI / MARINE', description: '桟橋を離れ、海からネオンの街を眺める。', color: '#79bbff', maxSpeed: 14, acceleration: 4, turnRate: .85, radius: 2.5, home: [55, 79] },
  { id: 'air', name: 'スカイポッド', tag: 'SORA / AIR', description: '屋根の上へ。空から港と山を巡る二人乗り。', color: '#f898ff', maxSpeed: 20, acceleration: 7, turnRate: 1.25, radius: 1.8, home: [70, 23] },
];
...
export function stepDrive(state:DriveState, vehicle:VehicleDefinition, input:{throttle:number;steer:number;vertical:number;brake:boolean}, dt:number, obstacles:Collider[]):DriveState {
  ...
  const acceleration=(input.brake?vehicle.acceleration*3:input.throttle===0?vehicle.acceleration*.55:vehicle.acceleration)*dt;
  let speed=state.speed+Math.max(-acceleration,Math.min(acceleration,target-state.speed));
  const steering=(state.steering??0)+(input.steer-(state.steering??0))*(1-Math.exp(-dt*7));
  const grip=vehicle.id==='boat'?.65:vehicle.id==='truck'?.5:.35;
  ...
  if(vehicle.id==='air') {
    const nextY=Math.max(groundHeight(z,x)+2.2,Math.min(74,state.y+input.vertical*9*dt));
  ...
  if(canDrive(vehicle.id,nx,nz,nextY,obstacles)){x=nx;z=nz;y=nextY;}else speed=0;
...
export function sampleRoute(route:readonly RoutePoint[],distance:number):{x:number;z:number;heading:number}{
  ...
  const total=lengths.reduce((a,b)=>a+b,0);if(total===0)return {x:route[0][0],z:route[0][1],heading:0};
  let remaining=((distance%total)+total)%total;
  for(let i=0;i<route.length;i++){if(remaining<=lengths[i]&&lengths[i]>0){const a=route[i],b=route[(i+1)%route.length],t=remaining/lengths[i];return{x:a[0]+(b[0]-a[0])*t,z:a[1]+(b[1]-a[1])*t,heading:Math.atan2(-(b[0]-a[0]),-(b[1]-a[1]))};}remaining-=lengths[i];}
