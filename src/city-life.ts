...
export class CityLife {
  ...
  private material(color:string,glow=false){const key=color+glow;if(!this.materials.has(key))this.materials.set(key,new THREE.MeshStandardMaterial({color,roughness:.45,metalness:.25,emissive:glow?color:'#000000',emissiveIntensity:glow?2.8:0}));return this.materials.get(key)!;}
  ...
  private label(parent:THREE.Object3D,text:string,sub:string,x:number,y:number,z:number,width:number,color:string){const c=document.createElement('canvas');c.width=512;c.height=192;const ctx=c.getContext('2d')!;ctx.fillStyle='rgba(8,14,35,.78)';ctx.fillRect(0,0,512,192);ctx.strokeStyle=color;ctx.lineWidth=4;ctx.strokeRect(3,3,506,186);ctx.textAlign='center';ctx.fillStyle=color;ctx.shadowColor=color;ctx.shadowBlur=13;ctx.font='600 54px sans-serif';ctx.fillText(text,256,88);ctx.font='22px monospace';ctx.fillText(sub,256,144);const map=new THREE.CanvasTexture(c);map.colorSpace=THREE.SRGBColorSpace;const mesh=new THREE.Mesh(new THREE.PlaneGeometry(width,width*.375),new THREE.MeshBasicMaterial({map,transparent:true,side:THREE.DoubleSide,depthWrite:false,toneMapped:false}));mesh.position.set(x,y,z);parent.add(mesh);return mesh;}
  vehicleModel(kind:VehicleKind,color:string){
    const g=new THREE.Group(),dark='#182534',glass='#244d68';g.userData.wheels=[];g.userData.rotors=[];
    if(kind==='bike'){
      this.box(g,.75,.45,2.6,0,.85,0,color);this.box(g,.65,.2,1.2,0,1.16,.3,dark);this.box(g,1.45,.1,.12,0,1.65,-.85,dark);this.box(g,.16,.6,.16,0,1.35,-.8,dark);this.box(g,.5,.12,.12,0,1.05,-1.35,'#d0ffff',true);
      for(const z of [-.9,.9]){const ring=new THREE.Mesh(new THREE.TorusGeometry(.43,.12,8,16),this.material('#57ffe1',true));ring.rotation.y=Math.PI/2;ring.position.set(0,.48,z);g.add(ring);}this.halo(g,0,.3,0,3,color);const rider=new THREE.Group();this.box(rider,.52,.65,.36,0,1.65,.2,'#4e6981');this.sphere(rider,0,2.2,.08,.27,.27,.26,'#25374b');this.box(rider,.4,.1,.06,0,2.23,-.16,'#a3ffe8',true);for(const x of [-.32,.32]){const arm=this.box(rider,.14,.65,.14,x,1.68,-.15,'#4e6981');arm.rotation.x=-.8;this.box(rider,.19,.56,.24,x,1.02,.5,'#27384c');}g.add(rider);g.userData.rider=rider;
    }else if(kind==='truck'){
      ...
      for(const x of [-.65,.65])this.box(g,.4,.18,.1,x,.85,-1.9,'#e2ffd8',true);this.box(g,1.4,.06,.04,0,2.18,-1.67,color,true);
      ...
      this.box(g,1.1,.13,.1,0,1,-1.9,'#edffff',true);this.box(g,.9,.09,.09,0,1,2,color,true);
    }
    if(kind==='truck'){
      ...
      this.box(g,.4,.08,.08,0,1,1.33,'#ff7a89',true);g.scale.setScalar(.88);
    }else if(kind==='boat'){
  ...
  private person(index:number,robot:boolean){const g=new THREE.Group();const color=robot?['#c0d8d3','#b797c0','#e6bb73'][index%3]:['#69629b','#2d8b91','#a2576f','#bb8055','#486c88'][index%5];const limbs:THREE.Object3D[]=[];
    if(robot){this.box(g,.65,.65,.5,0,.95,0,color);this.box(g,.52,.4,.45,0,1.52,0,'#263a50');this.box(g,.37,.09,.04,0,1.53,-.24,'#60fff0',true);this.box(g,.12,.4,.12,0,1.9,0,color);this.sphere(g,0,2.12,0,.08,.08,.08,'#ff97ed',true);this.box(g,.37,.32,.35,0,.87,-.4,'#bc9870');}
    else{this.box(g,.48,.7,.33,0,1.1,0,color);this.box(g,.4,.24,.32,0,.71,0,'#243247');this.sphere(g,0,1.69,0,.22,.25,.21,'#b99985');this.sphere(g,0,1.82,.025,.24,.15,.23,'#29303e');if(index%3===0)this.box(g,.41,.09,.06,0,1.72,-.21,'#76eaff',true);this.box(g,.3,.4,.19,0,1.12,.27,'#253347');this.box(g,.3,.045,.05,0,1.25,-.18,index%2?'#f584df':'#60ffdc',true);}
  ...
  private buildAtmosphere(){
    for(const x of [-70,-35,0,35,70])for(let z=-100;z<25;z+=7){this.box(this.scene,.075,.04,2.7,x+3.2,groundHeight(z)+.09,z,'#47dacd',true);}
    for(let i=0;i<14;i++){
      const x=-62+(i%5)*35,z=9-Math.floor(i/5)*39,y=groundHeight(z)+8;
      const g=new THREE.Group();g.position.set(x,y,z+6);const color=i%2?'#ff73cd':'#61eeff';
      this.label(g,['義体修理','夜市 2089','海上都市','電脳茶屋','汐凪回線'][i%5],['CYBERNETICS','NIGHT MARKET','FLOATING FUTURES','NEURAL TEA','CONNECTED COAST'][i%5],0,0,0,4.7,color);
      this.halo(g,0,0,.1,7,color);this.scene.add(g);this.holograms.push(g);
      this.box(this.scene,.12,5,.12,x-2.5,y-1,z+6,color,true);
      // 地面の淡い光で、既存の建物にもネオンの色を残す。
      const pool=new THREE.Mesh(new THREE.PlaneGeometry(7,5),new THREE.MeshBasicMaterial({map:this.haloTexture,color,transparent:true,opacity:.32,blending:THREE.AdditiveBlending,depthWrite:false}));pool.rotation.x=-Math.PI/2;pool.position.set(x,groundHeight(z+10)+.15,z+10);this.scene.add(pool);
    }
    for(const [x,z,color] of [[-15,19,'#f071d1'],[17,19,'#48ead7'],[0,-53,'#79acff']] as const){const light=new THREE.PointLight(color,50,25,1.5);light.position.set(x,5+groundHeight(z),z);this.scene.add(light);}
    // 海の町の目印として、巨大な魚の立体広告を港の上に浮かべる。
    const fish=new THREE.Group();fish.position.set(13,23,28);
    const holo=new THREE.MeshBasicMaterial({color:'#60ffe7',wireframe:true,transparent:true,opacity:.6,depthWrite:false});const body=new THREE.Mesh(new THREE.SphereGeometry(1,16,10),holo);body.scale.set(5,1.7,1.5);fish.add(body);
    const tail=new THREE.Mesh(new THREE.ConeGeometry(2.4,3,3),holo);tail.rotation.z=-Math.PI/2;tail.position.x=-5.6;fish.add(tail);this.sphere(fish,3.1,.5,1,.18,.18,.18,'#ffb0f1',true);this.label(fish,'汐凪夜市','FRESH CATCH / FRESH FUTURE',0,-3,0,9,'#87ffe5');this.halo(fish,0,0,0,18,'#299b98');this.scene.add(fish);fish.userData.fish=true;this.holograms.push(fish);
    const beam=new THREE.Mesh(new THREE.CylinderGeometry(6,.2,21,24,1,true),new THREE.MeshBasicMaterial({color:'#43ffc9',transparent:true,opacity:.025,side:THREE.DoubleSide,depthWrite:false,blending:THREE.AdditiveBlending}));beam.position.set(13,10,28);this.scene.add(beam);
    for(let i=0;i<7;i++){
      const x=-133+(i%2)*17,z=-65-i*15,h=28+(i%3)*12,y=groundHeight(z);this.box(this.scene,10,h,12,x,y+h/2,z,'#263749');
      this.box(this.scene,.16,h+5,.16,x+5,y+h/2,z+6,'#be64ef',true);for(let f=0;f<h/3;f++)for(let k=0;k<4;k++)this.box(this.scene,1.25,.65,.07,x-3.5+k*2.2,y+2+f*3,z+6.1,(f+k)%4?'#387d96':'#d27bc4',true);
    }
    this.label(this.scene,'汐凪モビリティ','LAND / SEA / SKY · V',0,7.3,19,9,'#a3ffe4');
  }
