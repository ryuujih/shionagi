...
import { HOUSE_PALETTE, harborNight, applyTimeLook, harborGlow, harborMat, SURFACE, ACCENT } from './look.ts';
...
export class HarborWorld {
  ...
  constructor(private container: HTMLDivElement, onStatus: (s: WorldStatus) => void, onLock: (locked: boolean) => void) {
    ...
    this.renderer.toneMappingExposure = harborNight.exposure;
    this.container.appendChild(this.renderer.domElement);
    this.scene.fog = new THREE.FogExp2(harborNight.fog, harborNight.fogDensity);
    this.build();
  ...
  private mat(color: string | number, roughness = 0.78) { return harborMat(color, roughness); }
  private glow(color: string, strength = 1.6) {
    const m = harborGlow(color, strength);
  ...
  private textSign(text: string, sub: string, w: number, h: number, x: number, y: number, z: number, color: string = ACCENT.teal, parent: THREE.Object3D = this.scene) {
    const c = document.createElement('canvas'); c.width = 768; c.height = 256;
    const ctx = c.getContext('2d')!; ctx.fillStyle = SURFACE.labelFillWorld; ctx.fillRect(0,0,768,256);
  ...
  private build() {
    this.sky = new THREE.ShaderMaterial({ side: THREE.BackSide, depthWrite:false, uniforms:{ top:{value:new THREE.Color(harborNight.skyTop)}, bottom:{value:new THREE.Color(harborNight.skyBottom)} }, vertexShader:'varying vec3 v; void main(){v=position;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}', fragmentShader:'varying vec3 v; uniform vec3 top; uniform vec3 bottom; void main(){float h=normalize(v).y; gl_FragColor=vec4(mix(bottom,top,smoothstep(-.05,.65,h)),1.);}' });
    this.scene.add(new THREE.Mesh(new THREE.SphereGeometry(950,32,16),this.sky));
    this.ambient = new THREE.HemisphereLight(harborNight.ambientSky,harborNight.ambientGround,harborNight.ambientIntensity); this.scene.add(this.ambient);
    this.sun = new THREE.DirectionalLight(harborNight.sunColor,harborNight.sunIntensity); this.sun.position.set(-120,180,90); this.scene.add(this.sun);
    ...
    const mountainMat = this.mat(SURFACE.baseMid);
    for(let i=0;i<17;i++){
    ...
    this.scene.add(new THREE.Mesh(landGeo,this.mat(SURFACE.ground)));
    this.box(192,3,3,0,-1.5,32,this.mat('#63706b'));
    const roadMat=this.mat('#2a3d40',0.45); const curb=this.mat('#6a7870');
    ...
    this.water = new THREE.ShaderMaterial({ uniforms: { time:{value:0}, waterColor:{value:new THREE.Color(harborNight.water)} }, transparent:false,
      ...
      fragmentShader:`varying vec3 p; uniform float time; uniform vec3 waterColor; void main(){float wave=sin(p.x*.25+p.y*.45+time)*sin(p.y*.23-time*.8); float fine=pow(max(0.,sin(p.x*.7+p.y*1.8+time*.7)),18.); float strip=pow(max(0.,sin(p.x*.046+sin(p.y*.08)*.23)),28.); vec3 col=waterColor+wave*.018+fine*.045; col+=vec3(.22,.38,.30)*strip*(.12+fine*.8)*exp(-abs(p.y+70.)*.005); gl_FragColor=vec4(col,1.); }` });
    ...
    this.textSign('汐凪漁港','SHIONAGI PORT / SINCE 1968',11,3.4,-50,5,30,ACCENT.teal);
    ...
    for(let i=0;i<3;i++){const drone=new THREE.Group();const m=this.mat('#96b5b3');this.box(1.5,.3,.65,0,0,0,m,drone);this.box(.7,.12,.15,0,-.2,.4,this.glow(ACCENT.teal),drone);this.scene.add(drone);drone.userData.flying=true;drone.userData.offset=i*2.1;this.drones.push(drone);}
  ...
  private buildHouse(x:number,z:number,w:number,d:number,h:number,color:string,name:string[],n:number){
    ...
    const windowMat=this.glow(n%3===0?ACCENT.teal:ACCENT.amber,n%3===0?.9:.65);
    ...
    this.box(w+.4,.14,2.1,x,y+3.4,front+.9,this.mat(n%2?SURFACE.wetGray:SURFACE.rustAmberDeep));
    this.textSign(name[0],name[1],Math.min(w-1,8),1.75,x,y+4.6,front+.22,n%3===0?ACCENT.teal:ACCENT.amber);
    this.box(.11,h,.13,x-w/2+.25,y+h/2,front+.15,this.glow(n%2?ACCENT.teal:ACCENT.amber,1.25));
    if(n%3===0){
      const g=new THREE.Group();g.position.set(x+w/2+.18,y+5.6,front-1.5);g.rotation.y=Math.PI/2;this.textSign(n%2?'宿':'酒','OPEN',2,2.4,0,0,0,ACCENT.amber,g);this.scene.add(g);
    }
    ...
    if(n%4===0){
      ...
      this.box(.97,1.4,.05,x-w/2-.75,y+1.6,front-.24,this.glow(ACCENT.teal,.8));
    ...
    if(n%2===0){const lantern=new THREE.Mesh(new THREE.SphereGeometry(.45,10,8),this.glow(ACCENT.amber,1.4));lantern.scale.y=1.4;lantern.position.set(x-w*.4,y+2.7,front+1);this.scene.add(lantern);}
  }
  private lamp(x:number,z:number,y:number){
    const m=this.mat(SURFACE.wetGray);this.box(.13,6.4,.13,x,y+3.2,z,m);this.box(1.7,.13,.13,x+.8,y+6.3,z,m);this.box(.9,.12,.6,x+1.2,y+6.2,z,this.glow(ACCENT.amber,1.55));
    const glow=new THREE.Mesh(new THREE.PlaneGeometry(5,5),new THREE.MeshBasicMaterial({color:ACCENT.amber,transparent:true,opacity:.04,depthWrite:false}));glow.rotation.x=-Math.PI/2;glow.position.set(x+1,y+.08,z);this.scene.add(glow);
    const light=new THREE.PointLight(ACCENT.amber,28,18,1.6);light.position.set(x+1.1,y+6.1,z);this.scene.add(light);
  }
  private boat(x:number,z:number,n:number){
    ...
    this.box(2.6,.16,6,0,.8,-.7,this.mat('#506e70'),group);this.box(2.1,1.8,2.6,0,1.6,-1.1,hull,group);this.box(1.7,.85,.06,0,1.9,.23,this.glow(ACCENT.teal,.5),group);this.box(2.4,.16,2.9,0,2.55,-1.1,hull,group);this.box(.09,4.5,.09,0,3,-1.5,hull,group);this.box(2.5,.05,.05,0,4.7,-1.5,hull,group);
    for(let i=0;i<3;i++)this.box(.55,.4,.8,-.6+i*.6,1,1.5,this.mat('#6f9c9b'),group);
    const bulb=new THREE.Mesh(new THREE.SphereGeometry(.12,8,8),this.glow(ACCENT.amber,2));bulb.position.set(0,5.1,-1.5);group.add(bulb);this.scene.add(group);group.userData.phase=n;this.boats.push(group);
  }
  ...
  setNight(night:boolean){applyTimeLook(night,{sky:this.sky,ambient:this.ambient,sun:this.sun,scene:this.scene,water:this.water,renderer:this.renderer});}
