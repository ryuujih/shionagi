...
export const MISSIONS = [
  { title:'帰港灯のない夜', task:'港で整備士ナギの依頼を聞く', x:8,z:23,kind:'talk',event:'accept',speaker:'ナギ / 港の整備士',text:'帰港灯が消えた。自動操船の船が、沖で立ち往生している。配達員のレン、あんたの足を貸して。夜市に預けた中継コアを受け取って、山灯の旧式中継局へ運んでほしい。',action:'依頼を引き受ける' },
  { title:'夜市の落とし物', task:'東浜夜市の保管端末をスキャンする',x:180,z:20,kind:'scan',event:'core',speaker:'保管端末',text:'ナギからの荷物を確認。中継コアを回収した。配送ログには、灯台から送られた不審な停止命令が残っている。',action:'コアを回収する' },
  { title:'山灯の古い約束', task:'山灯の中継局にコアを設置する',x:35,z:-174,kind:'scan',event:'relay',speaker:'ナギ / 無線',text:'旧回線がつながった！ 原因は青汐灯台の保安ドローン。誤った封鎖命令を繰り返している。EMPパルスで3機を停止して。住民には当てないでね。',action:'中継局を復旧する' },
  { title:'灯台の番人', task:'青汐島の暴走ドローン3機をEMPで停止する',x:263,z:260,kind:'combat',event:'drones',speaker:'ナギ / 無線',text:'3機とも停止。制御キーを回収した。潮路ターミナルに行けば、港湾ネットワークを再起動できる。',action:'ドローンを停止する' },
  { title:'誰のための航路', task:'潮路ターミナルで復旧方針を選ぶ',x:288,z:-30,kind:'choice',event:'network',speaker:'ミオ / 港湾システム技師',text:'故障じゃない。運営会社が「採算の悪い島」を航路から外そうとしたんだ。航路だけ戻せば、監視網は残る。監視網ごと切り離せば、これからは島の人たちが自分で守ることになる。レン、どうする？',action:'復旧方針を選ぶ' },
  { title:'おかえり、汐凪', task:'港に戻ってナギに報告する',x:8,z:23,kind:'talk',event:'home',speaker:'ナギ / 港の整備士',text:'見て、帰港灯が戻った。沖の船も動き出してる。今日の配達、ちゃんと届いたね。ありがとう、レン。夜明けまで、まだ少し時間がある。',action:'依頼を完了する' },
] as const;
