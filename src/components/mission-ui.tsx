...
export function MissionUI(p:Props){
  ...
  return <>
    ...
    {s.dialogue&&mission&&<div className="absolute inset-0 z-50 flex items-end justify-center bg-[#071520]/55 p-5 pb-12 backdrop-blur-[2px]"><section role="dialog" aria-label="シナリオ会話" aria-modal="true" className="flex w-full max-w-2xl gap-5 rounded-md border border-teal-100/25 bg-[#102435]/96 p-6 shadow-xl sm:p-7"><FacePanel id={mission.portrait} name={mission.speaker}/><div className="min-w-0 flex-1"><p className="text-[10px] tracking-[.18em] text-teal-200/80">{mission.roleTitle}</p><h2 className="mt-2 text-xl tracking-wider">{mission.title}</h2><p className="mt-3 text-sm leading-8 text-white/85">{mission.text}</p>{mission.kind==='choice'?<div className="mt-5 grid gap-3 sm:grid-cols-2"><Button onClick={()=>p.onConfirm('free')}>監視網を切り離す</Button><Button variant="outline" onClick={()=>p.onConfirm('routes')}>航路だけ復旧する</Button></div>:<Button className="mt-5 w-full" onClick={()=>p.onConfirm()}>{mission.action}<ArrowRight size={16}/></Button>}</div></section></div>}
