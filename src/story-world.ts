...
export type StorySnapshot={enabled:boolean;stage:number;choice:EndingChoice|null;hp:number;ammo:number;reloading:boolean;scan:number;kills:number;near:boolean;dialogue:boolean;down:boolean;locked:boolean;message:string;saveAvailable:boolean};
export const EMPTY_STORY:StorySnapshot={enabled:false,stage:0,choice:null,hp:100,ammo:12,reloading:false,scan:0,kills:0,near:false,dialogue:false,down:false,locked:false,message:'',saveAvailable:true};
...
export class StoryWorld {
  ...
  private message='';private messageUntil=0;private saveAvailable=true;
  ...
  constructor(private scene:THREE.Scene,private colliders:Collider[]){
    ...
    const ring=new THREE.Mesh(new THREE.TorusGeometry(.7,.05,8,24),this.mat('#f5ca7f',true));ring.rotation.x=Math.PI/2;this.marker.add(ring);const diamond=new THREE.Mesh(new THREE.OctahedronGeometry(.4),this.mat('#f5ca7f',true));diamond.position.y=3;this.marker.add(diamond);this.scene.add(this.marker);this.marker.visible=false;
  ...
  explore(){this.enabled=false;this.dialogue=false;this.messageUntil=0;this.resetFight();this.traces.forEach(t=>{t.life=0;t.line.visible=false;});}
  ...
  snapshot(pos:THREE.Vector3):StorySnapshot{return{enabled:this.enabled,stage:this.progress.stage,choice:this.progress.choice,hp:Math.ceil(this.hp),ammo:this.ammo,reloading:this.reloadTime>0,scan:this.scan/2.5,kills:this.enemies.filter(e=>e.hp===0).length,near:this.near(pos),dialogue:this.dialogue,down:this.down,locked:!!this.locked,message:this.clock<this.messageUntil?this.message:'',saveAvailable:this.saveAvailable};}
  clearLock(){this.locked=null;}
  target(){return this.locked?.hp?this.locked.model.position:null;}
  toggleLock(pos:THREE.Vector3){if(this.locked){this.locked=null;return;}this.locked=this.enemies.filter(e=>e.hp>0&&e.model.visible&&e.model.position.distanceTo(pos)<65&&this.clearLine(pos,e.model.position)).sort((a,b)=>a.model.position.distanceToSquared(pos)-b.model.position.distanceToSquared(pos))[0]??null;}
  reload(){if(this.ammo<12&&this.reloadTime<=0&&!this.down)this.reloadTime=1.3;}
  dodge(){if(this.invulnerable<=0&&!this.down)this.invulnerable=.28;}
  ...
  fire(camera:THREE.PerspectiveCamera,pos:THREE.Vector3){
    ...
    if(hit){hit.hp--;this.message=hit.hp?'EMP命中':'ドローン停止';this.messageUntil=this.clock+1.3;if(hit.hp<=0){hit.model.visible=false;if(this.locked===hit)this.locked=null;}}
    if(this.progress.stage===3&&this.enemies.every(e=>e.hp===0))this.finish();
  }
  private resetFight(){this.hp=100;this.ammo=12;this.down=false;this.reloadTime=0;this.fireCooldown=0;this.scan=0;this.scanning=false;this.locked=null;this.spawned=false;this.enemies.forEach(e=>{e.hp=2;e.charge=0;e.model.visible=false;});this.bolts.forEach(b=>{b.life=0;b.mesh.visible=false;});}
  ...
  update(dt:number,pos:THREE.Vector3,heading:number,speed:number,active:boolean,onFoot:boolean,playing:boolean){
    ...
    if(this.scanning){if(!onFoot||!this.near(pos)){this.scanning=false;this.scan=0;}else{this.scan+=dt;if(this.scan>=2.5)this.finish();}}
