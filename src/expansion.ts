...
export class WorldExpansion {
  ...
  private house(x:number,z:number,w:number,d:number,h:number,color:string,label?:string){const y=groundHeight(z,x);this.box(w,h,d,x,y+h/2,z,color);this.colliders.push({x,z,w,d,height:h+1});this.box(w+.6,.4,d+.6,x,y+h,z,'#304e5b');
    ...
    const floors=Math.floor(h/3);for(let f=0;f<floors;f++)for(let k=0;k<3;k++){const wx=x-w*.3+k*w*.3,wy=y+1.8+f*3;this.box(w*.21,1.4,.07,wx,wy,z+d/2+.04,(f+k)%3?'#d4b581':'#75bdc5',true);this.box(.06,1.4,.09,wx,wy,z+d/2+.1,'#314f58');if(f>0)this.box(w*.25,.12,.7,wx,wy-.8,z+d/2+.25,'#718c88');}
    ...
    for(const side of [-1,1]){this.box(.08,1.3,2,x+side*(w/2+.05),y+2,z,'#baae86',true);this.box(.1,h,.1,x+side*(w/2-.15),y+h/2,z+d/2+.12,'#64bdb0',true);}
  ...
  private stall(x:number,z:number,name:string,color:string){const y=groundHeight(z,x);this.box(5,.9,2.3,x,y+.45,z,color);this.box(5.4,.12,2.7,x,y+2.9,z,'#9f6758');for(const dx of [-2.35,2.35])this.box(.13,3,.13,x+dx,y+1.5,z,'#777c69');this.sign(name,'FRESH / LOCAL',x,y+2.35,z+1.3,3.8,'#ffcb97');
    for(let k=0;k<5;k++){this.box(.65,.15,.85,x-1.7+k*.83,y+1,z,'#80a6a1');this.sphere(x-1.7+k*.83,y+1.13,z,.26,.08,.1,'#b6cecf');}
    this.lantern(x+2,y+2.4,z+1.2);for(let k=0;k<9;k++)this.steamBases.push(x+.15*Math.sin(k),y+1.1+k*.15,z-.5);this.colliders.push({x,z,w:5,d:2.3,height:1});
  }
  private eastTown(){
    ...
    // 夜市は中央通路を空けた、実際に入れる屋根付き広場。
    this.box(39,.4,36,180,6.6,0,'#394e5b');for(const x of [161,199])for(const z of [-17,17]){this.box(.35,6.4,.35,x,3.2,z,'#8a9d91');this.colliders.push({x,z,w:.4,d:.4,height:7});}
    for(const z of [-10,0,10]){this.stall(166,z,'海の幸','#477e82');this.stall(194,z,z===0?'潮ラーメン':'焼き魚','#8f6257');}
    for(const z of [-12,0,12])this.box(34,.09,.14,180,6.3,z,'#f2b57f',true);
    const marketLight=new THREE.PointLight('#ffb17b',90,40,1.5);marketLight.position.set(180,4.7,0);this.scene.add(marketLight);
    this.sign('東浜 夜市','NIGHT MARKET / OPEN UNTIL DAWN',180,5,18.2,14,'#ffabca');
    for(let x=164;x<=196;x+=4)this.lantern(x,5.8,15,x%8?'#ebaa75':'#da8bbb');
    ...
    const material=new THREE.MeshStandardMaterial({color:'#b9c3cc'});const train=new THREE.Mesh(new THREE.BoxGeometry(14,3.2,3),material);this.tram.add(train);for(let x=-5;x<=5;x+=2){const window=new THREE.Mesh(new THREE.BoxGeometry(1.4,1.5,.06),new THREE.MeshBasicMaterial({color:'#79cdda'}));window.position.set(x,.2,1.54);this.tram.add(window);}this.tram.position.set(220,18,-38);this.scene.add(this.tram);
